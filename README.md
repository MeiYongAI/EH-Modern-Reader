...
- ⚡ **智能预加载** - 前后页轻量预加载，降低等待
- 💾 **进度记忆** - 自动保存阅读位置（刷新继续）
- 🧠 **图片缓存** - 已看图片再次跳转瞬间显示，避免重复请求
- 🚫 **防连跳抖动** - 快速拖动进度条或连点时只加载最终停留页
- 🔍 **缩略图悬停模式** - 开关控制：开启后鼠标靠近底部才显示缩略图与进度条
- ⌨️ **快捷键控制** - 翻页、缩放、重置一键到位
- 📱 **响应式设计** - 适配多分辨率与窗口大小
- 🖼️ **多适配模式** - contain / width / height / 原始大小
- 📑 **阅读模式选择（实验）** - 单页 / 纵向连续 / 横向连续（后续增强）
- 🎯 **无缝替换原版阅读器** - 自动注入，不破坏原画廊页面
...
- **平滑滚动** - 缩略图滚动动画效果
- **阅读模式** - 单页 / 纵向连续 / 横向连续（当前仅记录设置，后续扩展多页渲染）
- **缩略图悬停开关** - 关闭：底部菜单隐藏；开启：靠近底部显示缩略图与进度条
...
1. **连续阅读模式** 尚未真正渲染所有页面，仅作为设置占位
2. **图片真实 URL 解析** 仍依赖页面抓取，未来考虑使用官方 API
3. **极端快速滚动** 下仍可能触发多次预加载（已做合并处理）
4. **跨域限制** 某些镜像服务器可能阻止获取原图
