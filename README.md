...
提示：
- 在横向连续模式下，使用“进度条”或“页码输入”进行跳转时，将瞬时定位到目标页（不进行平滑滚动）；
- 缩略图点击与键盘翻页在横向模式下仍为平滑滚动，以保持滚动体验的一致性。
...
- 进度条与页码输入：
  - 单页模式：直接加载并显示该页
  - 横向连续模式：瞬时跳转到目标页位置（不进行平滑滚动）
- 键盘与缩略图：
  - 单页模式：逐页切换
  - 横向连续模式：平滑滚动至目标页附近

以上策略在拖动或频繁跳转时能减少等待，同时保留横向模式的连续滚动手感。

### 进度与历史

- 自动保存：阅读时会自动保存以下信息（每个画廊独立）：
  - 最后阅读页码
  - 阅读模式（单页 / 横向连续）
  - 横向连续模式下的滚动位置（scrollLeft）
  - 时间戳
- 自动恢复：再次打开同一画廊时会自动恢复阅读模式；
  - 若是横向连续模式并存在滚动位置，将恢复滚动位置；
  - 若没有保存滚动位置，则瞬时跳转到保存的页码。
- 历史列表：维护最近 200 条阅读记录，便于后续在 UI 中加入历史入口（当前先行保存）。
