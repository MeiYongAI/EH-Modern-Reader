# Release Notes - v2.0.0# Release Notes



## 🎉 EH Modern Reader v2.0.0 - 正式发行版## 🎉 EH Modern Reader v1.2.0 - 完美居中 & 稳定增强
...
这是 EH Modern Reader 的首个正式发行版本，完整支持 Gallery 模式、请求节流防封禁、项目规范化，标志着项目进入成熟稳定阶段。这是 EH Modern Reader 的 1.2.0 正式版本，带来缩略图系统重构、三区点击导航、强化的初始化稳定性等重大更新。
...
### 🎨 Gallery 模式### 🖼️ 缩略图系统全面重构

**无需 300 Hath，人人可用**采用独立真实图片 Canvas 渲染，彻底解决顶部空白问题，实现完美居中。



- **一键启动** - 画廊页面（`/g/`）点击"EH Modern Reader"按钮即可**技术亮点**

- **完整功能** - 支持单页/横向连续模式、预加载、进度记忆等全部功能- **固定占位容器** - 100×142 固定尺寸，防止布局跳动

- **智能节流** - 内置风控机制，安全使用- **雪碧图快速预览** - 利用站点原始图作为即时背景，零延迟
...
- **滚动锁** - 跳页时锁定 2 秒，防止洪水请求- 首屏即刻展示预览，无白屏等待

- **批量加载** - 可视区域优先，最多 10 张/次

### 🖱️ 三区点击导航

**实测效果**：全新的点击区域划分，操作更直观。

- ✅ 正常翻页：无风控触发

- ✅ 大幅跳页（1→100）：手动批量加载，无IP封禁**交互设计**

- ✅ 连续使用数小时：稳定无封禁- **左侧 1/3** - 向左翻页

- **中间 1/3** - 切换顶栏显示/隐藏

### 🎨 UI/UX 优化- **右侧 1/3** - 向右翻页
...
- **图片填充** - `width/height: 100%` + `object-fit: contain`，无留白- 覆盖整个视图区域

- **菜单切换** - header 绝对定位，无图片跳动- 移除原有的小圆翻页按钮



#### 启动按钮### 🛡️ 初始化稳定性增强

- 去除渐变背景，与站点原生风格统一三层兜底机制，彻底解决"无法加载图片列表"问题。
...
#### 目录结构重组3. **HTTP 回退** - 直接抓取页面 HTML 并解析 `imagelist`
...
├── scripts/       # 5个构建脚本（build、generate-icons等）- 减少用户刷新页面次数

├── dist/          # 发布包- 提升首次加载成功率

├── content.js     # MPV 核心 (2740+ 行)

├── gallery.js     # Gallery 启动 (420 行)### ⚡ 性能优化

├── style/         # 样式文件- **预测性预加载** - 横向模式检测滚轮方向，提前加载前方 4 页

├── icons/         # 扩展图标- **CORS 优化** - 避免 Canvas 污染，直接插入节点
...
- ✅ 删除 10+ 个冗余/过期文件- **响应式优化** - 宽度 92%，内边距减少到 18px/20px

- ✅ 删除 3 个空目录（`src/`, `js/`, `dist/test-extract/`）
...
- ✅ CHANGELOG.md - 完整版本记录- 真实图片独立缩略图渲染（Canvas contain+center）

- ✅ scripts/README.md - 脚本使用说明- 三区点击导航（左右翻页，中间切换顶栏）
...
1. 下载 [v2.0.0 发布包](https://github.com/MeiYongAI/eh-reader-extension/releases/tag/v2.0.0)- 设置面板最大宽度: 420px → 360px

2. 解压 ZIP 文件- 设置面板响应式宽度: 90% → 92%

3. 打开 `chrome://extensions/` 或 `edge://extensions/`- 设置面板内边距: 20px/24px → 18px/20px
...
2. 解压 ZIP 文件- 缩略图跳转后位置错误

3. 打开 `about:debugging#/runtime/this-firefox`- 设置弹窗在小屏设备上过宽
...
### Gallery 模式（手动）1. 删除旧版扩展

1. 访问画廊页面（`https://e-hentai.org/g/...`）2. 下载最新版本

2. 点击右侧"EH Modern Reader"按钮3. 重新加载扩展
