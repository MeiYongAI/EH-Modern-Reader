...
## [2.3.6] - 2026-01-21
### Added
- **油猴脚本版本**：新增 Tampermonkey/Violentmonkey 版本，功能与浏览器扩展版 100% 对等。
  - 自动构建脚本 `scripts/build-userscript.ps1`，将 content.js 和 gallery.js 合并为单文件油猴脚本。
  - 存储 API 适配层，将 `chrome.storage` 自动适配为 `GM_setValue/GM_getValue`。
  - 完整文档：`dist/INSTALL.md`（安装指南）、`dist/README_USERSCRIPT.md`（功能说明）、`dist/TECHNICAL.md`（技术细节）。
  - 生成的脚本文件：`dist/eh-modern-reader.user.js`（190.6 KB，4670 行代码）。
- **纵向连续阅读模式**：新增"纵向连续"阅读模式，支持上下滚动浏览所有页面。
- **单页竖向模式**：新增"单页竖向"阅读模式，使用上下方向键/滚轮翻页。
- **纵向连续自动滚动**：自动播放按钮在纵向连续模式下支持持续滚动。
- **纵向侧边边距可调**：设置面板新增纵向连续模式侧边边距(px)输入，范围 0-1000（v2.3.6 后期更新从 400 提升至 1000）。
...
### Fixed
- 修复读模式刷新后跳转到第一张图的问题（现在正确恢复到上次阅读页）。
